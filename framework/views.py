...
def evidence_list(request, identifier):
    ...
    return render(request, 'framework/evidence_list.html',
                  {'list': evidences, 'info': user, 'assessments': assessments})
...
def evidence_new(request, identifier):
    ...
    if request.method == 'POST':
        try:
            ...
            form = EvidenceForm()
            context = {'form': form, 'error': 'Este principio ya está registrado para la estrategia', 'info': user,
